...
public class SecondBinaryAlignerTestSetEvaluator 
{
	...
	static final String testAlignment = alignmentFolder + "/test_alignment_binary_ext_bin.csv";
	...
	public static void main(String[] args) 
	{
		try
		{
			...
			{
				SecondBinaryAligner aligner = new SecondBinaryAligner(1.0, 0.6, 0.0+i, measuresFile, testAlignment);
				aligner.startAligning();
				collectAllAlignments();
				
				SecondBinaryAlignerTestSetEvaluator testSetEvaluator = new SecondBinaryAlignerTestSetEvaluator();
				...
				//System.out.print(i + ": " + "(" + String.format("%.3g", recall).replace(',','.') + ","+
	...
	public SecondBinaryAlignerTestSetEvaluator() throws IOException
	{
			...
			try
			{
				...
				if (!groundedAlignment.contains(current_a))
				{
					...
					if (expectedAlignment != null)
					{
						...
						if (expAlignResult.compareTo("rel") == 0)
						{
